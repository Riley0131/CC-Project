...
def sortUrls(urls):
    #Sort the URL from the modules based on type
    youtube = []
    canvas = []
    other = []
...
def main():
    ...
    for course in courses_ids:
        ...
        with open(f'data/modules_{course}.json', 'w') as f:
            json.dump(modules, f, indent=4)


    #Sort URLs 
    for course in courses_ids:
        with open(f'data/modules_{course}.json', 'r') as f:
            urls = json.load(f)
        sortedUrls = sortUrls(urls)
        with open(f'data/sorted_modules_{course}.json', 'w') as f:
